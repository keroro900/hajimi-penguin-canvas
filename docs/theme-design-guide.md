...
| `saint-seiya` | 圣域、星座、小宇宙 | 圣斗士风格，强调十二宫、黄金圣衣、星图、水晶感 UI 和冥界篇隐藏模式。 |
| `tetris` | 方块、街机、消行 | 俄罗斯方块主题，强调方块拼接画布、方块节点框、方块 MiniMap 和可玩的右上角小游戏。 |
| `farm-story` | 牧场、种植、建造、手账 | 牧场物语主题，强调木牌节点、牧场地图底纹、农具端口、麻绳 / 水渠 / 田埂连线、镰刀剪线、牧场 HUD 和全画布养成层。 |
...
SHIFT 划线断连也是主题的一部分。新增官方主题时，除了节点、端口和背景，也要为 `--t8-cut-cursor`、`--t8-cut-button-mask`、`--t8-cut-color` 提供主题化切断符号，例如 OP 用弯刀、RH 用断链、火影用手里剑、EVA 用 Prog Knife、幽游白书用灵剑、灌篮高手用篮球 / 哨子、足球小将用足球 / 哨子、七龙珠用四星球 / 能量星、俄罗斯方块用像素断块、牧场物语用镰刀 / 剪枝剪，避免所有主题都显示默认剪刀。
...
| 字段 | 可选值 | 说明 |
|---|---|---|
| `style` | `plain` / `tech` / `pixel` / `op` / `rh` / `naruto` / `eva` / `yyh` / `slamdunk` / `soccer-hero` / `dragon-ball` / `saint-seiya` / `tetris` / `farm-story` | 选择视觉皮肤。 |
| `intensity` | `subtle` / `medium` / `strong` | 装饰强度。不是所有皮肤都会用满三档。 |
| `iconPack` | `default` / `op` / `naruto` / `eva` / `yyh` / `slamdunk` / `soccer` / `dragon-ball` / `saint-seiya` / `tetromino-well` / `farm-tools` | 图标包。牧场物语可用 `farm-tools`，俄罗斯方块可用 `tetromino-well`。 |
| `canvasPattern` | `none` / `dots` / `map` / `circuit` / `confetti` / `hub` / `chakra` / `eva-grid` / `spirit-map` / `court` / `pitch` / `dragon-radar` / `sanctuary-zodiac` / `tetris-stack` / `pasture-map` | 画布纹理倾向。牧场物语使用 `pasture-map`，俄罗斯方块使用 `tetris-stack`。 |
| `nodeFrame` | `plain` / `glass` / `sticker` / `wanted` / `hub-card` / `shinobi-scroll` / `eva-panel` / `spirit-case` / `scoreboard-card` / `match-card` / `capsule-card` / `cloth-box-card` / `arcade-cabinet-card` / `farm-sign-card` | 节点外框倾向。牧场物语使用 `farm-sign-card`，俄罗斯方块使用 `arcade-cabinet-card`。 |
...
| 星座圣域 | `pixel` | `saint-seiya` | `sanctuary-zodiac` | `cloth-box-card` |
| 方块街机 | `pixel` | `tetris` | `tetris-stack` | `arcade-cabinet-card` |
| 牧场手账 | `pixel` | `farm-story` | `pasture-map` | `farm-sign-card` |
...
|---|---|---|
| `title` | 文本 | 音乐名。 |
| `preset` | `tech-pulse` / `pixel-pop` / `grand-line-adventure` / `rh-pulse` / `shinobi-flame` / `eva-sync` / `spirit-gun` / `buzzer-beater` / `golden-goal` / `ki-burst` / `shenron-aura` / `pegasus-cosmos` / `hades-eclipse` / `block-drop` / `farm-breeze` | 合成音乐预设或音乐气质。 |
