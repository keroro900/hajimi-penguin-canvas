...
| `soft` | 柔和、浮雕、新拟物 | Soft UI 风格，内外双阴影、胶囊按钮、雾面卡片，适合设置面板、小工具和个人效率工具，长时间使用也舒适。 |
| `wabi` | 侘寂、纸纤维、水墨 | 侘寂 Wabi-Sabi 风格，低饱和灰绿灰褐、纸纤维纹理、手作纸卡、水墨连线，克制留白呈现宁静与时间感，适合文化/工艺/茶室/冥想叙事。 |
| `vapor` | 蒸汽波、紫黑霓虹、日落网格 | Vaporwave 80/90s 赛博怀旧风格，紫黑霓虹底、日落透视网格地面、粉青双色霓虹描边与极轻辉光，克制霓虹动效（仅边与选中节点辉光呼吸），SHIFT 切断用故障色块三色偏移，适合赛博/复古/夜间叙事。 |
| `utility` | 实用主义、网格、靛蓝、工程化 | Utility-first 设计系统示例板风格，中性灰底 + 1 个靛蓝品牌色 + 可见 40px 方格网格 + 浅阴影（0 2px 8px 10%）+ 方角圆角（6-8px），纯静态克制动效（140ms 仅 border/bg/shadow 微调，无辉光无 keyframes），SHIFT 切断用工程化细直线，适合设计系统样例、后台工具、文档/组件库示例。 |
| `skeuo` | 拟物化、金属面板、木质底座、LED | Skeuomorphism 拟物化风格，金属面板（冷色拉丝渐变 + 顶部高光 + 外阴影）+ 木质底座（暖棕木纹条纹）混搭，凸起按键按下回弹有重量感（140ms 回弹 cubic-bezier），选中节点用琥珀 LED 指示灯 2s 呼吸发光，连线选中琥珀辉光，SHIFT 切断用黄铜刻刀，适合音乐播放器、调音台、复古仪表盘、专业控制面板、精密工具仪表界面。 |
| `retro` | 复古 OS、teal 桌面、3D bevel、深蓝标题栏 | Retro OS 90 年代桌面审美，青绿色桌面背景（teal 固定纯色）+ 灰色窗口/任务栏 + 深蓝标题栏渐变 + 经典 3D 凸起边框（上亮下暗 bevel 双线），按钮按下时边框翻转成凹陷 + 1px 位移，端口是方角小方块（非圆），连线深蓝选中变红虚线，几乎无动效（80ms linear 直给，仅边流动 1.2s），SHIFT 切断用红色像素剪刀，适合怀旧主题网站、复古游戏平台、技术博客「虚拟桌面」入口、模拟器/工具类产品操作系统皮肤。 |
| `ink` | 水墨意境、墨分五色、朱红印章、书法字体 | 中国传统水墨美学，墨分五色（焦浓重淡清）+ 朱红印章（#C41E3A）+ 青绿山水（#2E8B57）+ 金箔点缀（#D4AF37）+ 书法字体（Ma Shan Zheng 行书/ZCOOL XiaoWei 楷书/Zhi Mang Xing 行草/ZCOOL KuaiLe 篆刻/Noto Serif SC 衬线）。画布如宣纸册页（固定纯色 + 纤维纹理 + 墨晕），节点如手卷题跋（朱红淡晕标题栏 + 行书标题 + 右上角朱红方印选中态），连线如书法笔触（浓墨 round cap + 选中朱红），端口朱红小圆点像印章纽，SHIFT 切断用毛笔枯笔飞白（断续 dasharray），动效 320ms ease-out 墨晕扩散缓入缓出，适合文化机构、茶道品牌、艺术平台、养生健康等需要传达内涵与品位的产品。 |
| `tap-studio` | 黑底创作台、玻璃节点、柔彩状态 | 黑底 AI 创作台风格，低亮深色画布 + 42px 微网格 + 蓝色操作焦点（#1FA2DC/#33A8FF）+ 粉/绿柔彩状态反馈。节点使用半透明玻璃卡片、细边框、轻辉光选中态，连线选中后变成克制流动虚线，SHIFT 切断使用蓝粉绿三线切片，适合密集图片/视频/音频 AI 工作流和夜间创作。 |
...
主题识别度来自重复出现的“视觉母题”，例如 OP 风格使用海图、悬赏令、草帽、赤红、海蓝、金币金；RH 风格使用黑绿工作台、荧光运行态、云工作流网格和无边框卡片；火影忍者风格使用木叶护额、卷轴纸、查克拉蓝、火焰橙红和忍术阵纹理；EVA 风格使用初号机紫、荧光同步绿、警戒橙红、MAGI 雷达、AT Field 几何和装甲面板；幽游白书风格使用灵丸能量、霓虹灵界地图、暗紫青绿终端、REI MAP 小地图和战斗卡片；灌篮高手风格使用木地板球场、红黑球衣、计分牌、传球弧线、战术板和篮球橙；足球小将风格使用绿茵球场、蓝白球衣、比分牌和传球轨迹；七龙珠风格使用胶囊公司设备、神龙雷达、七颗龙珠徽章、橙金气功波和深蓝科技反差；圣斗士风格使用十二宫、星图、水晶边框和小宇宙；俄罗斯方块主题使用方块井、像素街机、彩块和消行反馈；牧场物语主题使用草地格、耕地、木牌、农具、麦穗、麻绳、水渠、牧场手账和季节徽章；像素糖果风使用硬描边、贴纸卡片、糖果色、圆胶囊和硬阴影。

SHIFT 划线断连也是主题的一部分。新增官方主题时，除了节点、端口和背景，也要为 `--t8-cut-cursor`、`--t8-cut-button-mask`、`--t8-cut-color` 提供主题化切断符号，例如 OP 用弯刀、RH 用断链、火影用手里剑、EVA 用 Prog Knife、幽游白书用灵剑、灌篮高手用篮球 / 哨子、足球小将用足球 / 哨子、七龙珠用四星球 / 能量星、俄罗斯方块用像素断块、牧场物语用镰刀 / 剪枝剪、蒸汽波用故障色块（紫粉青三色偏移的小色条 mask）、实用主义用工程化细直线（靛蓝 1.6px 水平线）、拟物化用黄铜刻刀（暖色细线 + 双色阴影偏移），避免所有主题都显示默认剪刀。
...
| 字段 | 可选值 | 说明 |
|---|---|---|
| `style` | `plain` / `tech` / `pixel` / `rh` / `soft` / `wabi` / `vapor` / `utility` / `skeuo` / `retro` / `ink` / `tap-studio` | 选择视觉皮肤。 |
| `intensity` | `subtle` / `medium` / `strong` | 装饰强度。不是所有皮肤都会用满三档。 |
| `iconPack` | `default` | 图标包。当前只保留默认线性图标。 |
| `canvasPattern` | `none` / `dots` / `map` / `circuit` / `confetti` / `hub` / `soft-dots` / `wabi-paper` / `vapor-grid` / `utility-grid` / `skeuo-wood` / `retro-desktop` / `ink-paper` / `tap-void` | 画布纹理倾向。Soft UI 使用 `soft-dots`，侘寂使用 `wabi-paper`，蒸汽波使用 `vapor-grid`，实用主义使用 `utility-grid`，拟物化使用 `skeuo-wood`，复古 OS 使用 `retro-desktop`，水墨意境使用 `ink-paper`，黑底流光使用 `tap-void`。 |
| `nodeFrame` | `plain` / `glass` / `sticker` / `hub-card` / `soft-card` / `wabi-card` / `vapor-card` / `utility-card` / `skeuo-panel` / `retro-window` / `ink-scroll` / `tap-glass` | 节点外框倾向。Soft UI 使用 `soft-card`，侘寂使用 `wabi-card`，蒸汽波使用 `vapor-card`，实用主义使用 `utility-card`，拟物化使用 `skeuo-panel`，复古 OS 使用 `retro-window`，水墨意境使用 `ink-scroll`，黑底流光使用 `tap-glass`。 |
...
| 柔和浮雕 | `tech` | `soft` | `soft-dots` | `soft-card` |
| 侘寂纸境 | `tech` | `wabi` | `wabi-paper` | `wabi-card` |
| 蒸汽波 | `tech` | `vapor` | `vapor-grid` | `vapor-card` |
| 实用主义 | `tech` | `utility` | `utility-grid` | `utility-card` |
| 拟物化 | `tech` | `skeuo` | `skeuo-wood` | `skeuo-panel` |
| 复古 OS | `tech` | `retro` | `retro-desktop` | `retro-window` |
| 水墨意境 | `tech` | `ink` | `ink-paper` | `ink-scroll` |
| 黑底流光 | `tech` | `tap-studio` | `tap-void` | `tap-glass` |
...
|---|---|---|
| `title` | 文本 | 音乐名。 |
| `preset` | `tech-pulse` / `pixel-pop` / `grand-line-adventure` / `rh-pulse` / `shinobi-flame` / `eva-sync` / `spirit-gun` / `buzzer-beater` / `golden-goal` / `ki-burst` / `shenron-aura` / `pegasus-cosmos` / `hades-eclipse` / `block-drop` / `farm-breeze` / `tap-flow` | 合成音乐预设或音乐气质。 |
...
2. 选一个基础视觉：`tech`、`pixel`、`op`、`rh`、`naruto`、`eva`、`yyh`、`slamdunk`、`soccer-hero`、`dragon-ball`、`saint-seiya`、`tetris`、`farm-story` 或 `plain`。
...
如果你想新增一种真正的视觉风格，而不是只用现有 `plain/tech/pixel/op/rh/naruto/eva/yyh/slamdunk/soccer-hero/dragon-ball/saint-seiya/tetris/farm-story` 调色，需要改代码。
