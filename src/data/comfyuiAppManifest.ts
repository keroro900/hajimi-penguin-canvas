...
export const COMFYUI_APP_MANIFEST: ComfyAppManifest = {
  ...
  apps: [
    ...
    },
    {
      ...
      title: '内容包食谱样例',
      categoryId: 'general',
      description: 'content-pack-v2：字段映射、排除规则、缺模型、缺节点处理的公开示例，运行前替换本机模型。',
      workflowJson: createBasicComfyTextToImageWorkflow(),
      fields: [
        { nodeId: '1', fieldName: 'ckpt_name', source: 'ckpt_name' },
        { nodeId: '2', fieldName: 'text', source: 'prompt' },
      ...
      userParams: [
        { key: 'ckpt-name', label: 'Checkpoint', kind: 'text', source: 'ckpt_name', defaultValue: '请改成你的模型.safetensors', required: true },
        { key: 'prompt', label: '正向 Prompt', kind: 'textarea', source: 'prompt', defaultValue: 'content-pack-v2 product photo recipe, clean lighting', rows: 5 },
        { key: 'negative', label: '负向 Prompt', kind: 'textarea', source: 'negative', defaultValue: 'low quality, watermark, random text', rows: 4 },
        { key: 'width', label: '宽度', kind: 'number', source: 'width', defaultValue: 1024, min: 64, max: 8192, step: 8 },
        { key: 'height', label: '高度', kind: 'number', source: 'height', defaultValue: 1024, min: 64, max: 8192, step: 8 },
      ],
      outputs: [{ key: 'image', label: '输出图', kind: 'image' }],
      capabilities: ['content-pack-v2', 'image.generate', 'sample.workflow'],
      runtime: { pollIntervalMs: 1000, maxPolls: 3600 },
      ui: { icon: 'BookOpen', accent: '#22c55e' },
      version: 1,
      updatedAt: '2026-06-12',
    },
    {
      id: 'content-recipes-panorama-video',
      title: '全景转视频食谱样例',
      categoryId: 'video',
      description: 'content-pack-v2：把 3D 全景、首帧和运镜说明组合成可导入的 ComfyUI 字段映射示例；缺模型或缺节点时先按文档替换本机依赖。',
      workflowJson: createBasicComfyTextToImageWorkflow(),
      fields: [
        { nodeId: '1', fieldName: 'ckpt_name', source: 'ckpt_name' },
        { nodeId: '2', fieldName: 'text', source: 'prompt' },
      ...
      userParams: [
        { key: 'ckpt-name', label: 'Checkpoint', kind: 'text', source: 'ckpt_name', defaultValue: '请改成你的模型.safetensors', required: true },
        { key: 'prompt', label: '正向 Prompt', kind: 'textarea', source: 'prompt', defaultValue: 'content-pack-v2 panorama to video recipe, stable camera path notes', rows: 5 },
        { key: 'negative', label: '负向 Prompt', kind: 'textarea', source: 'negative', defaultValue: 'low quality, visible stitch line, flicker, missing node', rows: 4 },
        { key: 'width', label: '宽度', kind: 'number', source: 'width', defaultValue: 1536, min: 64, max: 8192, step: 8 },
        { key: 'height', label: '高度', kind: 'number', source: 'height', defaultValue: 768, min: 64, max: 8192, step: 8 },
      ],
      outputs: [{ key: 'image', label: '输出帧', kind: 'image' }],
      capabilities: ['content-pack-v2', 'video.workflow', 'sample.workflow'],
      runtime: { pollIntervalMs: 1000, maxPolls: 3600 },
      ui: { icon: 'Clapperboard', accent: '#f59e0b' },
      version: 1,
      updatedAt: '2026-06-12',
    },
  ],
