...
- `hakimi_canvas_node_capabilities`: 查询某种节点支持的动作、可编辑字段、输入端口和结果类型。
- `hakimi_canvas_configure_generation`: 按画布模型注册表配置现有图像/视频节点，可选择立即运行。
- `hakimi_canvas_run_node`: 通过可视化事件流运行单个节点。
- `hakimi_canvas_read_node_result`: 回读节点状态、错误和真实结果 URL。
- `hakimi_canvas_undo_batch`: 使用 `operationBatchId` 撤销一次完整画布操作；检测到后续编辑时默认拒绝覆盖。
...
- 单节点生成：`hakimi_canvas_node_capabilities` -> `hakimi_canvas_configure_generation` -> `hakimi_canvas_run_node` -> `hakimi_canvas_read_node_result`
- 小步修补：`hakimi_agent_run_actions`
- 撤销本轮：读取 apply/actions 返回的 `operationBatchId`，调用 `hakimi_canvas_undo_batch`
...
```json
{
  ...
  "plan": {
    ...
    "layoutIntent": { "direction": "left-to-right", "columnGap": 380, "rowGap": 260 },
