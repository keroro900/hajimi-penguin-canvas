...
const { writeSmokeReadyMarker } = require('./smoke-ready.cjs');
...
function createMainWindow() {
  ...
  mainWindow.once('ready-to-show', () => {
    ...
    try {
      if (writeSmokeReadyMarker({ version: APP_VERSION })) dbgLog('[smoke] readiness marker written');
    } catch (error) {
      dbgLog(`[smoke] readiness marker failed: ${normalizeError(error)}`);
    }
  });
